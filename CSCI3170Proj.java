...
class CSCI3170Proj {
    ...
    public static void createTables(Connection con) {
        ...
        try {
            ...
            stmt.executeUpdate(
                    "create table user_category (ucid integer, max integer, period integer, primary key(ucid))");
            stmt.executeUpdate(
                    "create table user (uid char(12), name char(25), age integer, occupation char(20), ucid integer, primary key(uid))");
            stmt.executeUpdate("create table car_category (ccid integer, name char(20),primary key(ccid))");
            stmt.executeUpdate(
                    "create table car (callnum char(8), name char(10), manufacture date,time_rent integer, ccid integer, primary key(callnum))");
            stmt.executeUpdate(
                    "create table copy (callnum char(8), copynum integer, primary key(callnum, copynum))");
            // 'return' is a reserved word
            stmt.executeUpdate(
                    "create table rent (uid char(10), callnum char(8), copynum integer, checkout date, returning date, primary key(uid, callnum, copynum, checkout))");
            stmt.executeUpdate(
                    "create table produce (cname char(25), callnum char(8), primary key(cname, callnum))");
        } catch (SQLException e) {
            System.out.println(e);
        }
    }
    ...
    public static void searchCar(int choice, Connection con, String keyword) {
        ...
        try {
            Statement stmt = con.createStatement();
            String query = "SELECT C.callnum, C.name, c.ccname, D.cname, P.copynum FROM car_category c, car C, copy P, produce D WHERE c.ccid = C.ccid AND C.callnum = P.callnum AND C.callnum = D.callnum ORDER by C.callnum";
            ...
            while (result.next()) {
                ...
                if (choice == 1) {
                    ...
                    for (int i = 0; i < name.length(); i++) {
                        if (i + size - 1 >= name.length()) {
                            break;
                        }
                        if (name.substring(i, i + size - 1).equals(keyword)) {
                    ...
                    for (int i = 0; i < company.length(); i++) {
                        if (i + size - 1 >= company.length()) {
                            break;
                        }
                        if (company.substring(i, i + size - 1).equals(keyword)) {
                            System.out.println("|" + callnum + "|" + name + "|" + carCategory + "|" + company + "|"
                                    + copynum + "|");
                        }
                    }
    ...
    public static void showLoan(Connection con, String userID) {
        ...
        try {
            Statement stmt = con.createStatement();
            String query = "SELECT C.callnum, P.copynum, C.name, D.cname, R.checkout, R.returning FROM car C, copy P, produce D, rent R WHERE C.callnum = P.callnum AND C.callnum = D.callnum AND P.copynum = R.copynum AND R.uid = "
                    + userID + " ORDER by R.checkout DESC";
            ResultSet result = stmt.executeQuery(query);
            while (result.next()) {
                String callnum = result.getString("callnum");
                int copynum = result.getInt("copynum");
                ...
                java.sql.Date returning = result.getDate("returning");
    ...
    public static boolean rentCar(Connection con, String userID, String callNum, int copyNum) {
        try {
            Statement stmt = con.createStatement();
            String query = "SELECT * FROM rent WHERE returning = NULL AND callnum = " + callNum + " AND copynum = "
                    + copyNum;
            ResultSet result = stmt.executeQuery(query);
            if (result.next()) {
                return false;
    ...
    public static boolean returnCar(Connection con, String userID, String callNum, int copyNum) {
        try {
            ...
            if (result.next()) {
                ...
                java.util.Date returning = new Date();
                SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
                String qry = "UPDATE rent SET returning = " + formatter.format(returning)
                        + "WHERE returning = NULL AND callnum = " + callNum + " AND copynum = " + copyNum
                        + " AND uid = " + userID;
                stmt.executeUpdate(qry);
                return true;
            }
    ...
    public static void listUnreturned(Connection con, String startDate, String endDate) {
        ...
        try {
            Statement stmt = con.createStatement();
            String query = "SELECT uid, callnum, copynum, checkout FROM rent WHERE returning = NULL AND checkout >= "
